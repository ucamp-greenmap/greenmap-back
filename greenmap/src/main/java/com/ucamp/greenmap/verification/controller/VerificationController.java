...
public class VerificationController {
    ...
    // 따릉이 인증
    @PostMapping("/bike")
    public ResponseEntity<ApiResponse<VerificationResponse>> verifyBike(@AuthenticationPrincipal Long memberId, @RequestBody BikeRequest bikeRequest) {
        return ResponseEntity.ok(ApiResponse.success("인증에 성공했습니다", verificationService.verifyBike(memberId, bikeRequest)));
    }

    // 전기차/수소차 인증
    @PostMapping("/car")
    public ResponseEntity<ApiResponse<VerificationResponse>> verifyCar(@AuthenticationPrincipal Long memberId, @RequestBody CarRequest carRequest) {
    ...
    @PostMapping("/shop")
    public ResponseEntity<ApiResponse<VerificationResponse>> verifyShop(@AuthenticationPrincipal Long memberId, @RequestBody ShopRequest shopRequest) {
        return ResponseEntity.ok(ApiResponse.success("인증에 성공했습니다", verificationService.verifyShop(memberId, shopRequest)));
    }

    // 인증 내역 조회
    @GetMapping("/history")
    public ResponseEntity<ApiResponse<VerificationHistoryResponse>> getVerificationHistory(@AuthenticationPrincipal Long memberId) {
        return ResponseEntity.ok(ApiResponse.success("인증 내역 조회에 성공했습니다", verificationService.getVerificationHistory(memberId)));
    }

    // 이번달 인증 내역 조회
    @GetMapping("/month")
    public ResponseEntity<ApiResponse<MonthlyVerificationResponse>> getMonthlyVerification(@AuthenticationPrincipal Long memberId) {
